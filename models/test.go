...
var TestWhere = struct {
	...
	UserID:   whereHelperint{field: "`test`.`user_id`"},
	Name:     whereHelperstring{field: "`test`.`name`"},
	LastName: whereHelperstring{field: "`test`.`lastName`"},
}
...
var (
	testAllColumns            = []string{"user_id", "name", "lastName"}
	testColumnsWithoutDefault = []string{"name", "lastName"}
	testColumnsWithDefault    = []string{"user_id"}
	testPrimaryKeyColumns     = []string{"user_id"}
)

type (
	// TestSlice is an alias for a slice of pointers to Test.
	// This should generally be used opposed to []Test.
	TestSlice []*Test
...
func FindTest(ctx context.Context, exec boil.ContextExecutor, userID int, selectCols ...string) (*Test, error) {
	testObj := &Test{}
	...
	query := fmt.Sprintf(
		"select %s from `test` where `user_id`=?", sel,
	)
...
func (o *Test) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `test` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `test` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, testPrimaryKeyColumns))
		}
	...
	o.UserID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == testMapping["user_id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for test")
	}
...
func (o *Test) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "test", update, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for test")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for test")
	}
...
func (o *Test) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindTest(ctx, exec, o.UserID)
	if err != nil {
		return err
	}
...
func TestExists(ctx context.Context, exec boil.ContextExecutor, userID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `test` where `user_id`=? limit 1)"
