...
func testTestsExists(t *testing.T) {
	...
	e, err := TestExists(ctx, tx, o.UserID)
	if err != nil {
...
func testTestsFind(t *testing.T) {
	...
	testFound, err := FindTest(ctx, tx, o.UserID)
	if err != nil {
		t.Error(err)
	}
...
var (
	testDBTypes = map[string]string{`UserID`: `int`, `Name`: `varchar`, `LastName`: `varchar`}
	_           = bytes.MinRead
)
...
func testTestsUpsert(t *testing.T) {
	...
	if len(mySQLTestUniqueColumns) == 0 {
